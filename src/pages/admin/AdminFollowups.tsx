...
import { INTERESTED_DOMAIN_OPTIONS, Lead, Profile } from '@/types';
...
const AdminFollowups = () => {
  ...
  useEffect(() => {
    const fetchEmployeeNames = async () => {
      ...
      if (data) {
        ...
        data.forEach(p => { names[p.user_id] = p.name; });
  ...
  const followupLeads = useMemo(() => {
    return leads.filter(lead => {
      ...
      // Employee filter
      if (employeeFilter !== 'all' && lead.assigned_to !== employeeFilter) {
        return false;
      }
  ...
  return (
    <DashboardLayout requiredRole="admin">
      <div className="space-y-6">
        ...
        {/* Followup List Grouped by Employee */}
        <div className="space-y-6">
          {isLoading ? (
                  ...
                  <div className="divide-y divide-border">
                    {employeeLeads.map((lead) => {
                      ...
                      return (
                        <div 
                          key={lead.id} 
                          className={`py-4 cursor-pointer hover:bg-muted/50 transition-colors px-2 -mx-2 rounded ${isMaxedOut ? 'bg-red-50/50 dark:bg-red-950/20' : ''}`}
                              ...
                              <div className="flex items-center gap-3 text-xs text-muted-foreground">
                                ...
                                {lead.followup_date && (
                                  <span className="flex items-center gap-1">
                                    ...
                                    {format(new Date(lead.followup_date), 'MMM d, h:mm a')}
                                  </span>
                                )}
                              </div>
                            </div>
                            <div className="flex items-center gap-2">
                              <div className="flex items-center gap-1">
                                {Array.from({ length: MAX_FOLLOWUP_COUNT }).map((_, i) => (
                                  <div
                                    key={i}
                                    className={`w-1.5 h-1.5 rounded-full ${i < (lead.followup_count || 0) ? 'bg-amber-500' : 'bg-gray-200 dark:bg-gray-700'}`}
                                  />
                                ))}
                              </div>
